...
- **补偿策略**:
  - 错过 < 30 分钟：立即执行
  - 错过 30 分钟 -2 小时：标记为"补发"，执行简化流程
  - 错过 > 2 小时：记录日志，不执行，等待人工确认
- **验收标准**:
  - 错过任务在检测后 5 分钟内触发补偿
  - 补发任务有明确标识
  ...
  "tasks": [
    {
      ...
      "compensation": {
        "enabled": true,
        "window_minutes": 120,
        "simplified_threshold_minutes": 30
      },
      "execution": {
        "timeout_seconds": 300,
        "retry_count": 3,
...
│   └── scripts/
│       ├── cron_scheduler.sh     # 外部 cron 脚本（保留）
│       └── check_scheduler_status.sh  # 状态检查脚本
└── logs/
    └── scheduler/
        ├── scheduler.log         # 运行日志
...
# 手动触发任务
python3 Lamda-ai/HKTech-Agent/scheduler/scheduler.py --trigger premarket_learning
```
...
| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|---------|
| Agent 长时间不活跃 | 任务错过 | 中 | Heartbeat 定期检查 |
| sessions_spawn 失败 | 任务执行失败 | 低 | 重试机制 + 告警 |
| 状态文件损坏 | 状态丢失 | 低 | 定期备份 + 容错处理 |
| 配置错误 | 任务不执行 | 中 | 配置验证 + 启动检查 |
...
| 术语 | 定义 |
|------|------|
| 错过补偿 | 任务计划时间已过但未执行，自动触发的补救执行 |
| 简化流程 | 补发任务使用简化执行流程，减少耗时 |
| 补偿窗口 | 允许补发的时间范围（默认 2 小时） |
