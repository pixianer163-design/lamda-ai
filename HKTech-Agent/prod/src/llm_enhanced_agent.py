...
class LLMEnhancedAgent:
    ...
    def __init__(self, data_dir=None):
        ...
        if STRATEGY_ENGINE_AVAILABLE:
            try:
                ...
                print(
                    f"✅ 策略引擎: {capabilities['engine_type']} (VectorBT可用: {capabilities['vectorbt_available']})"
                )
            except Exception as e:
    ...
    def _load_portfolio(self) -> Dict:
        ...
        if os.path.exists(portfolio_file):
            with open(portfolio_file, "r") as f:
                return json.load(f)
        return {"cash": 19000, "holdings": {}, "total_value": 19000}
    ...
    def _load_market_data(self) -> Dict:
        ...
        try:
            ...
            if self._validate_market_data(market_data):
                print(
                    f"   ✅ 成功获取 {len(market_data)} 只股票实时数据 [数据源: {data_source_used}]"
                )
                return market_data
            else:
    ...
    def run_daily_analysis(self, news_items=None) -> dict:
        ...
        _log_performance(
            "load_market_data", step_duration, {"stocks_count": len(market_data)}
        )
        ...
        step_duration = time.time() - step_start
        _log_performance(
            "llm_signal_extraction", step_duration, {"has_news": news_items is not None}
        )
        ...
        try:
            ...
            base_decision = {
                "decisions": {
                    code: {"action": "hold", "confidence": 0.5} for code in market_data
                },
                "summary": f"策略引擎异常: {e}",
            }
        step_duration = time.time() - step_start
        ...
        decisions_map = (
            base_decision.get("decisions", base_decision)
            if isinstance(base_decision, dict)
            else base_decision
        )
        _log_performance(
            "base_strategy", step_duration, {"stocks_count": len(decisions_map)}
        )
        ...
        for code, dec in enhanced.get("final_decision", {}).items():
            ...
            _log_decision(
                code, dec["action"], dec.get("confidence", 0.5), reason, "llm_enhanced"
            )

        # 记录总体性能
        total_duration = time.time() - start_time
        _log_performance(
            "daily_analysis_total",
            total_duration,
            {
                "stocks_analyzed": len(market_data),
                "steps": 5,
                "prediction_enabled": prediction is not None
                and prediction.get("enabled", False),
            },
        )
        ...
        try:
            ...
            decisions_summary = {
                ...
                "world_model": {
                    ...
                    "recommendation": result.get("prediction", {}).get(
                        "recommendation", ""
                    ),
    ...
    def _base_strategy(self, market_data: Dict, prediction: Optional[Dict]) -> Dict:
        ...
        if prediction and prediction.get("enabled"):
            ...
            for code, decision in decisions.items():
                ...
                decisions[code] = {"action": action, "confidence": round(conf, 2)}
